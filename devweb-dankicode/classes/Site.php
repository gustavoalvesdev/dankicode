...
class Site 
{
	public static function updateUsuarioOnline()
	{
		if (isset($_SESSION['online'])) {
			...
			if ($check->rowCount() == 1) {
				...
				$token = $_SESSION['online'];
				$horarioAtual = date('Y-m-d H:i:s');
				$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin.online` (ip, ultima_acao, token) VALUES (?, ?, ?)");
	...
	public static function contador() 
	{
		if (!isset($_COOKIE['visita'])) {
			...
			$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin.visitas` (ip, dia) VALUES (?, ?)");
